...
assert.strictEqual(rows.length,251);
...
assert(rows.find(r=>r.sheet_name==='Sessions'&&r.column_name==='otp_plain').sensitive);
...
assert.strictEqual(result.created,true); assert.strictEqual(result.inserted,251); assert.strictEqual(context.ss.sheets.Handbook.getLastRow(),252);
...
assert.strictEqual(result.inserted,0); assert.strictEqual(result.updated,251); assert.strictEqual(result.custom_rows,1);
